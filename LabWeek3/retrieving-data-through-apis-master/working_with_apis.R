...
load("../tokens.Rdata")
census_api_key <- census_token#read_lines("census_api_key.txt")
...
subset(acs_df, grepl("Seattle", NAME))
...
prettify(content(r_za_pop, as = "text"))
...
prettify(content(r_za_pop_pg2, as = "text"))
...
head(multi_country_df)

ggplot(multi_country_df, aes(x = date, y = log(value))) + 
  geom_line() + 
  theme_minimal(base_size = 20) + 
